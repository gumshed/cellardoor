...
var uniswap_getClaimableMvt = async function(address){
	var stakingCont =  new web3.eth.Contract(uniswapMiningAbi, ENV.uniswapMiningAddress2);
...
var init_staking = async function(){
	...
	var lpCont =  new web3.eth.Contract(mvtLpAbi, ENV.lpAddress2);
	var stakingCont =  new web3.eth.Contract(uniswapMiningAbi, ENV.uniswapMiningAddress2);
...
var uniswap_prepare_stake = async function(){
	...
	var mvtCont =  new web3.eth.Contract(erc20Abi, ENV.mvtAddress);
	var lpCont =  new web3.eth.Contract(erc20Abi, ENV.lpAddress2);
	var miningCont =  new web3.eth.Contract(uniswapMiningAbi, ENV.uniswapMiningAddress2);
	...
	var allowance = await lpCont.methods.allowance(account, ENV.uniswapMiningAddress2).call();
	...
	if(web3.utils.fromWei(allowance)*1<stake_amount){ //allowance not enough, ask to approve
		...
		await lpCont.methods.approve(ENV.uniswapMiningAddress2, uintmax).send({from: account, gas: gasLimitApprove}, function(err, result){
...
var addLpToMetamask = async function(){
	...
	params: {
	  ...
	  options: {
		address: ENV.lpAddress2, // The address that the token is at.
